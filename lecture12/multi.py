...
#
Job = namedtuple('Job', ['name', 'run_time', 'working_set_size', 'affinity', 'time_left', 'prior_atio', 'deadline'])
...
class scheduler:
    def __init__(self, job_list, per_cpu_queues, affinity, peek_interval,
        ...
        if job_list == '':
            ...
            for j in range(job_num):
                ...
                prior_atio = int(random.random() * 100) if random.random() < 0.01 else (int(random.random() * 3) + 100)
                if job_list == '':
                    job_list = '%s:%d:%d:%d' % (str(j), run_time, working_set, prior_atio)
                else:
                    job_list += (',%s:%d:%d:%d' % (str(j), run_time, working_set, prior_atio))
        ...
        for entry in job_list.split(','):
            ...
            if len(tmp) != 4:
                ...
                exit(1)
            job_name, run_time, working_set_size, prior_atio = tmp[0], int(tmp[1]), int(tmp[2]), int(tmp[3])
            self.jobs[job_name] = Job(name=job_name, run_time=run_time, working_set_size=working_set_size, affinity=[], time_left=[run_time], prior_atio=prior_atio, deadline=0)
            print('Job name:%s run_time:%d working_set_size:%d prior_atio:%d' % (job_name, run_time, working_set_size, prior_atio))
    ...
    def assign_jobs(self):
        ...
        for cpu in cpu_list:
            if self.sched_state[cpu] == self.STATE_IDLE:
                self.get_job(cpu, self.single_sched_queue)
    ...
    def print_sched_queues(self):
        ...
        if self.per_cpu_queues:
            ...
            pass
            # print('Q: ', end='')
            # for job_name in self.single_sched_queue.lists[]:
            #     print('%s ' % job_name, end='')
            # print('    ', end='')
        return
    ...
    def run(self):
        ...
        while self.jobs_finished < self.num_jobs:
            ...
            # assign_job sign news jobs to CPUs (this can happen every tick?)
            self.assign_jobs()
