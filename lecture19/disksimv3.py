...
class Disk:
    def __init__(self, addr, addrDesc, lateAddr, lateAddrDesc,
        ...
        if self.policy == 'SCAN' or self.policy == 'CSCAN':
            self.currentDirect = True
        ...
        for (block, index) in rList:
            ...
            if self.currentDirect:
                ...
                if block <= self.currentBlock:
                    trackList.append((block, index))
        if trackList == []:
            if self.policy == 'CSCAN':
                minBlock  = -1
                minIndex  = -1
                for (block, index) in rList:
                    if self.requestState[index] == STATE_DONE:
                        continue
                    if minBlock == -1 or block < minBlock:
                        minBlock = block
                        minIndex = index
                if minBlock != -1:
                    trackList.append((minBlock, minIndex))
            else:
                self.currentDirect = not self.currentDirect
                for (block, index) in rList:
                    if self.requestState[index] == STATE_DONE:
                        continue
                    if self.currentDirect:
    ...
    def GetNextIO(self):
        ...
        elif self.policy == 'SCAN' or self.policy == 'CSCAN':
            # first, find all the blocks on a given track (given window constraints)
